(function() {
  ...
  callWithJQuery(function($) {
    ...
    $.pivotUtilities.subtotal_renderers = {
      "Таблица с подитогом": function(pvtData, opts) {
        return SubtotalRenderer(pvtData, opts);
      },
      "Таблица с подитогом Bar Chart": function(pvtData, opts) {
        return $(SubtotalRenderer(pvtData, opts)).barchart();
      },
      "Таблица с подитогом Тепловая карта": function(pvtData, opts) {
        return $(SubtotalRenderer(pvtData, opts)).heatmap("heatmap", opts);
      },
      "Таблица с подитогом Тепловая карта по строке": function(pvtData, opts) {
        return $(SubtotalRenderer(pvtData, opts)).heatmap("rowheatmap", opts);
      },
      "Таблица с подитогом Тепловая карта по столбцу": function(pvtData, opts) {
    ...
    return $.pivotUtilities.subtotal_aggregators = (function(tpl, sTpl) {
      return {
        "Сумма подитога по строке": sTpl.fractionOf(tpl.sum(), "row", usFmtPct),
        "Сумма подитога по столбцу": sTpl.fractionOf(tpl.sum(), "col", usFmtPct),
        "Подитог по строке": sTpl.fractionOf(tpl.count(), "row", usFmtPct),
        "Подитог по столбцу": sTpl.fractionOf(tpl.count(), "col", usFmtPct)
      };
